    ...
    */
    ...
    const handleClick = (event) => {
        ...
        if(validateEmail(input.username) && input.password.length>6){
           ...
           const result = axios.post("http://localhost:5000/Register", newUser).
           then(res => {
               console.log(res.ok);
                if(!res.ok){
    ...
    return (
        <div>
            ...
            <form class="inputField" action="/Register" method="POST">
                ...
                <button onClick={handleClick} type="submit" class="buttons btn btn-dark">Register</button>
            </form>
            <button class="loginbutton  btn btn-outline-dark btn-lg col-3" href="/auth/google" >Continue with Google <i class="fab fa-google"></i></button>
