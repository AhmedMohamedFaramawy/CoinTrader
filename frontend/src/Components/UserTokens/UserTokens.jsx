...
const UserTokens = () => {
  ...
  const getTokenCard = async () => {
    ...
    await axios.post("http://localhost:5000/UserTokens",{username: users}, {headers:{authorization:"Bearer "+localStorage.getItem("token")}})
      .then((response) => {
        setFilteredCards(response.data[0].createdCards);
        console.log("response.data[0].createdCards :",  response.data[0].createdCards);
      })
      .catch((err) => {
        console.error(err);
      });
  ...
  const handleCardName = async  (event) => {
    ...
    await axios.get(`http://localhost:5000/FilterUserTokens?username=${username}&cardName=${event.target.value}&category=${category}`,
  ...
  const handleCategory = async  (event) => {
    ...
    await axios.get(`http://localhost:5000/FilterUserTokens?username=${username}&cardName=${cardName}&category=${event.target.value}`,
      {headers:{authorization:"Bearer "+localStorage.getItem("token")}})
    .then((response) => {
      console.log(response);
      setFilteredCards(response.data);
  ...
  const handleFilter = async (event) => {
    ...
    await axios.get(`http://localhost:5000/FilterUserTokens?username=${username}&cardName=${cardName}&category=${category}`
    ,{headers:{authorization:"Bearer "+localStorage.getItem("token")}})
      .then((response) => {
  ...
  //const handleCardName = (event) => {
    ...
    //const users = tokens.username;
     ...
     //axios.get("http://localhost:5000/FilterUserTokens",
     //{username: users,
      //cardName: cardName,
      //category: category}, {headers:{authorization:"Bearer "+localStorage.getItem("token")}})
    //.then((response) => {
      //console.log(response);
      //setFilteredCards(response.data);
    //})
  ...
  const handleCardName = (event) => {
    ...
    //axios.get(`http://localhost:5000/FilteredUserTokens?search=${event.target.value}&category=${category}`)
     axios.post("http://localhost:5000/FilterUserTokens",
     {username: users,
      cardName: cardName,
      category: category}, {headers:{authorization:"Bearer "+localStorage.getItem("token")}})
    .then((response) => {
    ...
    //axios.get(`http://localhost:5000/FilteredUserTokens?search=${event.target.value}&category=${category}`)
     ...
     {username: users,
      ...
      .then((response) => {
        ...
        setFilteredCards(response.data[0].createdCards);
        console.log("response.data[0].createdCards", response.data[0].createdCards);
        console.log("response.data[0].createdCards[0]", response.data[0].createdCards[0]);
        //console.log("response.data.createdCards", response.data.createdCards);
        //console.log("response.data.createdCards[0]", response.data.createdCards[0]);
      })
      .catch((err) => {
        console.error(err);
      });
